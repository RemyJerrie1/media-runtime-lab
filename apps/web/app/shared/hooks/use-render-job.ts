...
import { createRenderJob, getRenderJob, renderJobEvents, type RenderEditorCommand } from '../api/render-jobs';
...
export function useRenderJob() {
  ...
  async function run(command:RenderEditorCommand){setBusy(true);setError(null);stream.current?.close();try{const created=await createRenderJob(command);setJob(created);connect(created.id,0);}catch(cause){setError(cause instanceof Error?cause.message:'Unable to create render job.');}finally{setBusy(false);}}

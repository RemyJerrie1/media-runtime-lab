...
export function RenderLab() {
  ...
  const [encoding,setEncoding]=useState<FfmpegEncoding>({codec:'libx264',preset:'medium',rateControl:'crf',crf:23,bitrateKbps:4000,gop:60,fps:30});
  const [processing,setProcessing]=useState<MediaProcessing>(defaults);
  const encode=(field:keyof Omit<FfmpegEncoding,'codec'>)=>(event:ChangeEvent<HTMLInputElement|HTMLSelectElement>)=>setEncoding(current=>({...current,[field]:field==='preset'||field==='rateControl'?event.target.value:Number(event.target.value)} as FfmpegEncoding));
  const process=(field:keyof MediaProcessing)=>(event:ChangeEvent<HTMLInputElement|HTMLSelectElement>)=>setProcessing(current=>({...current,[field]:event.target instanceof HTMLInputElement&&event.target.type==='checkbox'?event.target.checked:field==='audioSampleRate'?Number(event.target.value):event.target.value} as MediaProcessing));
  const keyframeSeconds=(encoding.gop/encoding.fps).toFixed(1);
  const rateArgs=encoding.rateControl==='crf'?`-crf ${encoding.crf}`:`-b:v ${encoding.bitrateKbps}k`;
  ...
  return <section className="lab" id="render-lab">
    <div className="lab-copy">
      ...
      <fieldset><legend>同步、水印與交付（Sync, Watermark & Delivery）</legend><div className="editor-grid">
        ...
        <label>字幕（Subtitles）<select aria-label="字幕模式" value={processing.subtitleMode} onChange={process('subtitleMode')}><option value="none">無（None）</option><option value="burn-in">燒錄字幕（Burn-in）</option><option value="webvtt">WebVTT 外掛字幕</option></select></label>
        <label>影音水印（Watermark）<select aria-label="影音水印模式" value={processing.watermarkMode} onChange={process('watermarkMode')}><option value="none">無（None）</option><option value="visible">可視水印（Visible）</option><option value="dynamic">動態鑑識水印（Dynamic Forensic）</option></select></label>
    ...
    <div className="console">
      <MediaTimeline />
      <div className="job"><MetricCard label="任務狀態（Job Status）" value={job?.status.toUpperCase()??'尚未開始' } tone={job?.status==='ready'?'success':'accent'}/><MetricCard label="處理階段（Stage）" value={job?.stage??'等待指令'}/><MetricCard label="成本／Token" value={job?`$${job.estimatedCostUsd} / ${job.tokens}`:'—'}/></div>
      <ProgressBar label="算圖進度（Render Progress）" value={job?.progress??0} tone={job?.status==='ready'?'success':'accent'}/>
      <div className="review-grid">
        <article><span>素材檢測（Probe）</span><strong>Codec · 解析度 · FPS · 長度 · Bitrate · Streams</strong><p>先檢測再決定串流複製（Stream Copy）或轉碼（Transcode），不可一律重編碼。</p></article>
        <article><span>GOP／跳轉（Seek）</span><strong>I/P/B 影格 · 隨機存取 · {keyframeSeconds} 秒間隔</strong><p>短 GOP 通常提升跳轉速度；長 GOP 通常有較佳壓縮效率。</p></article>
        <article><span>時間與同步（Time & Sync）</span><strong>PTS · DTS · Timebase · VFR/CFR · A/V Drift</strong><p>驗證時間戳單調遞增，並比較音訊與影片長度。</p></article>
        <article><span>播放（Playback）</span><strong>moov · Range · Buffer · Demux／Decode</strong><p>成品編碼成功，不代表啟播速度與緩衝體驗一定正常。</p></article>
        <article><span>影音水印（Watermark）</span><strong>可視 · 隱形 · 動態／鑑識</strong><p>此 Demo 規劃可視與動態疊加；高韌性的隱形水印需要獨立服務。</p></article>
        <article><span>廣告與交付（Ads & Delivery）</span><strong>CSAI · SSAI · HLS/DASH · CDN</strong><p>明確區分廣告決策、Manifest、追蹤、儲存與播放器責任。</p></article>
      </div>
      {job?<div className="encode-receipt" aria-live="polite"><span>後端處理收據（Backend Processing Receipt）</span><strong>{job.encoding.codec} · {job.encoding.fps}fps · CRF {job.encoding.crf} · GOP {job.encoding.gop} · {job.processing.frameRateMode.toUpperCase()}</strong><code>ffprobe {job.ffprobeArgs.join(' ')}</code><code>ffmpeg {job.ffmpegArgs.join(' ')}</code></div>:null}
    </div>
  </section>;
}

...
export class RenderJobAggregate {
  ...
  advance(status: RenderStatus, progress: number, stage: string): RenderJob {
    ...
    this.state = {
      ...this.state, status, progress, stage, sequence:this.state.sequence + 1, updatedAt:new Date().toISOString(),
      artifactUrl:ready ? `/artifacts/${this.state.id}.mp4` : this.state.artifactUrl,
      artifactChecksum:ready ? `sha256:${this.state.id.replaceAll('-','').padEnd(64,'0').slice(0,64)}` : this.state.artifactChecksum,
    };

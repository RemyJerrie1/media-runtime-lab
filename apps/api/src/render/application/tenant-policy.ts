...
export class TenantPolicy {
  ...
  authenticate(tenantId: string | undefined, apiKey: string | undefined) {
    ...
    if (!tenant) throw new UnauthorizedException('x-tenant-id is required');
    const expected = process.env.MEDIA_RUNTIME_API_KEY ?? 'local-demo-key';
    if (apiKey !== expected) throw new ForbiddenException('invalid tenant credential');
    return tenant;
  }
  rateLimit(tenantId: string) {
    const now = Date.now();
    const limit = Number(process.env.TENANT_RATE_LIMIT_PER_MINUTE ?? '30');

...
export class InMemoryWorkflowStore implements WorkflowStore {
  ...
  private jobKey(tenantId:string,id:string){ return `${tenantId}:${id}`; }
  async create({tenantId,traceId,command,quotaTokens}:CreateWorkflow){
    ...
    if(existingId){ return {job:structuredClone(this.jobs.get(this.jobKey(tenantId,existingId))!),created:false}; }
  ...
  async findById(tenantId:string,id:string){ const job=this.jobs.get(this.jobKey(tenantId,id)); return job?structuredClone(job):undefined; }
  async listEvents(tenantId:string,id:string,afterSequence:number){ return structuredClone((this.events.get(this.jobKey(tenantId,id))??[]).filter(event=>event.sequence>afterSequence)); }
  async claimNext(workerId:string,leaseMs:number){
    const now=Date.now(); const row=[...this.work.values()].find(item=>item.state==='pending'||(item.state==='leased'&&item.leaseUntil<now)); if(!row)return;
    row.state='leased'; row.workerId=workerId; row.leaseUntil=now+leaseMs; row.attempt+=1;
    const job=this.jobs.get(this.jobKey(row.tenantId,row.jobId)); if(job){job.attempt=row.attempt;}
  ...
  async advance(work:ClaimedWork,status:RenderStatus,progress:number,stage:string){
    const key=this.jobKey(work.tenantId,work.jobId); const current=this.jobs.get(key); const row=this.work.get(work.id);
    if(!current||!row||row.workerId!==work.workerId)return;
    const next=new RenderJobAggregate(current).advance(status,progress,stage); next.attempt=work.attempt; this.jobs.set(key,structuredClone(next));
    const event:RenderEvent={id:crypto.randomUUID(),jobId:next.id,tenantId:next.tenantId,sequence:next.sequence,type:'render.progress',data:structuredClone(next),createdAt:next.updatedAt};
    this.events.get(key)!.push(event);
    if(status==='ready'||status==='failed'){row.state='complete';row.workerId=undefined;row.leaseUntil=0;}
    else {row.state='leased';row.leaseUntil=Date.now()+5000;}
    return structuredClone(next);
  }
  async release(work:ClaimedWork){ const row=this.work.get(work.id); if(row&&row.workerId===work.workerId){row.state='pending';row.workerId=undefined;row.leaseUntil=0;} }
  async activeCount(tenantId:string){ return [...this.jobs.values()].filter(job=>job.tenantId===tenantId&&!['ready','failed'].includes(job.status)).length; }
}

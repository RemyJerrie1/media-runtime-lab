...
@Module({controllers:[RenderController],providers:[RenderOrchestrator,RenderWorker,OperationsTelemetry,TenantPolicy,{provide:WORKFLOW_STORE,useFactory:async():Promise<WorkflowStore>=>{const store:WorkflowStore=process.env.DATABASE_URL?new PostgresWorkflowStore(process.env.DATABASE_URL):new InMemoryWorkflowStore();await store.initialize();return store;}}]})

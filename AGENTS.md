...
4. Keep AI/provider code behind ports. Deterministic Canvas/FFmpeg composition remains available when provider output is late, invalid, or expensive.
5. A change is complete only after `pnpm verify`; snapshots may change only with an explicit reason in the commit.

## Review order

Read contract → domain invariant → application use case → adapter → UI. Reject cross-feature imports and hidden network calls in React components.
